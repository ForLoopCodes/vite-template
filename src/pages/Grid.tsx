...
export default function Grid(props: any) {
  ...
  const [cellSize] = useState(
    window.innerWidth >= 1200
      ? window.innerWidth >= 1200
        ? parseFloat(window.innerWidth.toFixed(2)) / 50
        : 40 / 50
      : 40
  );
  ...
  const [primaryColor, setPrimaryColor] = useState("bg-green-300"); // or bg-neutral-600
  const [secondaryColor, setSecondaryColor] = useState("bg-[#287050]"); // or bg-neutral-400
  ...
  const glowElement = (x: any, y: any, t1: any, t2: any) => {
  ...
  const glowElementAndHide = (x: any, y: any, t1: any, t2: any) => {
    const element = ref.current!.children[x * columns + y] || null;
    if (!element) return;
  ...
  const glowElementForever = (x: any, y: any) => {
    const element = ref.current!.children[x * columns + y] || null;
    if (!element) return;
    element.classList.add(primaryColor);
    element.classList.remove(backgroundColor);
  ...
  const gameOfLife = (rate: any) => {
    ...
    setTimeout(() => {
      const showLiveCells = (liveCells: any) => {
        unGlowAll();
  ...
  const firstOrderNuclearFission = (rate: any, density: any) => {
    // ----- DOCUMENTATION -----
  ...
  window.addEventListener("keydown", (e) => {
    if (e.key === "0") {
      ...
      const button = document.querySelector(".btn0") as HTMLButtonElement;
      button?.focus(); // Use optional chaining to avoid null reference error
    }
    if (e.key === "1") {
      matrixRain(500, 1, 100, 1200);
      const button = document.querySelector(".btn1") as HTMLButtonElement;
      button?.focus(); // Use optional chaining to avoid null reference error
    }
    if (e.key === "2") {
      gameOfLife(200);
      const button = document.querySelector(".btn2") as HTMLButtonElement;
      button?.focus(); // Use optional chaining to avoid null reference error
    }
    if (e.key === "3") {
      firstOrderNuclearFission(200, 50);
      const button = document.querySelector(".btn3") as HTMLButtonElement;
      button?.focus(); // Use optional chaining to avoid null reference error
    ...
    if (e.key === "4") {
      multiColor();
      const button = document.querySelector(".btn4") as HTMLButtonElement;
      button?.focus(); // Use optional chaining to avoid null reference error
    }
    if (e.key === "5") {
      snakeGame();
      const button = document.querySelector(".btn5") as HTMLButtonElement;
      button?.focus(); // Use optional chaining to avoid null reference error
    }
    if (e.key === "r") {
    ...
    <div>
      {window.innerWidth >= 1200 ? (
        <div
          ...
          style={{
            ...
            display: props.viewDetails ? "none" : "auto",
        ...
        >
          ...
          >
            <button
              className={`text-white font-bold py-2 px-4 h-full text-xl rounded ${backgroundColor} hover:bg-[#287050] focus:bg-[#287050] focus:outline-none focus:border-none btn0`}
              style={{
            ...
            <button
              className={`text-white font-bold py-2 px-4 h-full text-xl rounded ${backgroundColor} hover:bg-[#287050] focus:bg-[#287050] focus:outline-none focus:border-none btn1`}
              style={{
                width: `${cellSize * 3 + gap * (3 - 1)}px`,
                borderRadius: `${(cellSize * borderRadius) / 100}px`,
                fontFamily: "Space Mono",
            ...
            <button
              className={`text-white font-bold py-2 px-4 h-full text-xl rounded ${backgroundColor} hover:bg-[#287050] focus:bg-[#287050] focus:outline-none focus:border-none btn2`}
              style={{
                width: `${cellSize * 3 + gap * (3 - 1)}px`,
                borderRadius: `${(cellSize * borderRadius) / 100}px`,
                fontFamily: "Space Mono",
            ...
            <button
              className={`text-white font-bold py-2 px-4 h-full text-xl rounded ${backgroundColor} hover:bg-[#287050] focus:bg-[#287050] focus:outline-none focus:border-none btn3`}
              style={{
                width: `${cellSize * 3 + gap * (3 - 1)}px`,
                borderRadius: `${(cellSize * borderRadius) / 100}px`,
                fontFamily: "Space Mono",
            ...
            <button
              className={`text-white font-bold py-2 px-4 h-full text-xl rounded ${backgroundColor} hover:bg-[#287050] focus:bg-[#287050] focus:outline-none focus:border-none btn4`}
              style={{
                width: `${cellSize * 3 + gap * (3 - 1)}px`,
                borderRadius: `${(cellSize * borderRadius) / 100}px`,
                fontFamily: "Space Mono",
            ...
            <button
              className={`text-white font-bold py-2 px-4 h-full text-xl rounded ${backgroundColor} hover:bg-[#287050] focus:bg-[#287050] focus:outline-none focus:border-none btn5`}
              style={{
                width: `${cellSize * 3 + gap * (3 - 1)}px`,
                borderRadius: `${(cellSize * borderRadius) / 100}px`,
                fontFamily: "Space Mono",
          ...
          <button
            className={`text-white font-bold py-2 px-4 h-full text-xl rounded ${backgroundColor} hover:bg-[#287050] focus:bg-[#287050] focus:outline-none focus:border-none`}
            style={{
              width: `${cellSize * 3 + gap * (3 - 1)}px`,
              borderRadius: `${(cellSize * borderRadius) / 100}px`,
              fontFamily: "Space Mono",
      ...
      <div
        ...
        style={{
          ...
          gridGap: `${gap}px`,
          pointerEvents: props.viewDetails ? "all" : "auto", // changed for need, change "all" to "none if not needed"
        }}
      >
        {
